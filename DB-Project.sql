...
INSERT INTO `formazioni_geologiche` VALUES ('AI3YA','Vulcano sottomarino',1500,3,'La fessura emette lava in continuazione senza però effettuare violente eruzioni, dovrà essere monitorato costantemente'),('BQGI4','Vulcano sottomarino',800,4,'Il vulcano si è verificato essere attivo, grandi quantità di lava vengono rilasciate periodicamente senza però eventi violenti. Le città marittime più vicine sono state informate del possibile pericolo di una eruzione violenta ed è stato designato un piano di evacuazione.'),('C1AE0','Vulcano sottomarino',1000,2,'Il vulcano sembra inattivo, ma emana calore elevato, facendo presagire che possa attivarsi in un futuro prossimo, bisognerà monitorare l\'attività.'),('C7RIN','Grotta sottomarina',100,2,'La grotta è caratterizzata da stretti tunnel che rendono difficile l\'esplorazione. Le pareti e il soffitto della grotta sono composti da rocce porose caratterizzate da buchi con un diametro fino a 10 cm.'),('DXS44','Vulcano sottomarino',1000,2,'Il vulcano sembra essere parzialmente attivo, le emissioni di lava sono incostanti e scarse.'),('FOQ3N','Vulcano sottomarino',2800,4,'Chiamato anche Imperatore della Cina, il vulcano non erutta dal 1927 ed è stato molto attivo in tempi lontani, è necessario monitorarlo costantemente, in quanto un\'eruzione potrebbe causare violenti tsunami.'),('LAM2R','Grotta sottomarina',500,0,'La grotta è troppo stretta per essere esplorata, sono state trovate due entrate a distanza di circa 500m l\'una dall\'altra'),('M8L2Z','Fumarola nera',10,0,'La fumarola è attiva ed emette fumo costantemente ma non rappresenta un pericolo per l\'habitat circostante.'),('UQI12','Fumarola nera',5,0,'La fumarola emette fumo da 3 diversi punti ma è stata dichiarata innocua.'),('V4EP8','Vulcano sottomarino',300,1,'Il vulcano è stato confermato essere inattivo, probabilmente ha esaurito il magma nella camera magmatica. Sarà comunque necessario monitorarlo per controllare eventuali crolli della camera magmatica causati dalla pressione.'),('VX2AQ','Grotta sottomarina',300,0,'La grotta, accessibile dalla superficie, presenta numerose stalattiti sul soffitto. E\' caratterizzata da ampie camere rocciose, in alcune di esse le stalattiti e stalagmiti si congiungono creando delle colonne.'),('WU2QH','Fumarola nera',3,1,'La fumarola emette violentemente grandi quantità di fumo.'),('XCJ9Y','Grotta sottomarina',500,0,'La grotta è caratterizzata da numerosi e larghi tunnel che permettono di navigarla con sicurezza. Alcuni tunnel sono piastrellati da rocce trasportate da forti correnti o movimenti geologici.');
...
CREATE TABLE `gruppi_di_esplorazione` (
  ...
  UNIQUE KEY `ID_GRUPPO_DI_ESPLORAZIONE_IND` (`NomeAssociazione`,`Nome`),
...
INSERT INTO `gruppi_di_esplorazione` VALUES ('JAMSTEC','4XPCU'),('JAMSTEC','6G11M'),('JAMSTEC','A9MFE'),('NOAA','DEIJB'),('NOAA','QHK1T'),('NOAA','R67EF'),('Ocean Exploration Trust','WK8OQ'),('OceanX AG','ABB37'),('Odyssey Marine Exploration','R6ZLS');
...
INSERT INTO `luoghi` VALUES ('Oceano Indiano',NULL),('Fossa delle Aleutine','Alaska'),('Isole Galapagos','Ecuador'),('Baia Sagami','Giappone'),('Fossa del Giappone','Giappone'),('Mar di Banda','Indonesia'),('Golfo del Messico','Messico'),('Manus Basin','Papua Nuova Guinea');
...
CREATE TABLE `membri` (
  ...
  CONSTRAINT `EQU_MEMBR_GRUPP` FOREIGN KEY (`NomeAssociazione`, `IDgruppo`) REFERENCES `gruppi_di_esplorazione` (`NomeAssociazione`, `Nome`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `organismi` VALUES ('0F5W9M2W7L','Tiburonia granrojo',NULL,'Grande medusa rossa',2003,'Il nome di questa medusa deriva dai suoi colori rossi della campana (granrojo = grande rosso in spagnolo). Presenta una campana di grandi dimensioni, mentre i tentacoli sono più corti e tozzi.'),('364FIFD9JR','Atolla wyvillei',NULL,NULL,1880,'Questa specie di medusa presenta una campana rossa e un lungo tentacolo bianco che parte dal centro della campana e si allunga per una decina di centimetri'),('6567LBSV3P','Larvacean',NULL,NULL,1821,'I larvacei sono tunicati che abitano diverse zone degli oceani di tutto il mondo, per nutrirsi utilizzano una \"barriera\"\" di muco che filtra le sostanze nutritive.\"'),('7F0JSTSGR0','Magnapinna atlantica',NULL,NULL,2006,'Questi calamari sono caratterizzati, come gli altri membri del loro genere, da tentacoli molto lunghi, anche fino a 8 metri. Abitano le zone più profonde e oscure degli oceani, ancora non si sa di cosa si nutrono e le loro abitudini.'),('83SIM574N8','Hexatrygon bickelli',NULL,'Razza esabranchiata',1980,'Questa specie di razza presenta 6 paia di branchie, da cui deriva il suo nome. La testa è allungata e le dimensioni totali possono raggiungere anche 1.7m. Solitamente vivono a stretto contatto con il fondale marino.'),('A69M3QUS9A','Beroe forskalii',NULL,'Medusa pettine',2004,'Questa specie di ctenoforo di piccole dimensioni presenta un corpo completamente trasparente, fatta eccezione di filamenti \"dentati\"\" all\'interno del corpo\"'),('ACGEAS7P9A',NULL,'Pesce verticale',NULL,2023,'Questa specie di pesce è nota per la sua abitudine nel rimanere fermo in posizione verticale, con la testa rivolta verso l\'alto, nonostante il movimento in acqua è comune a tutti gli altri pesci.'),('AI4CEQC9PM',NULL,'Medusa quadri-tentacolo',NULL,2023,'Questa medusa di piccole dimensioni presenta 4 sottili tentacoli e una campana trasparente con un anello fluorescente. Utilizza la propulsione causata dal movimento della campana per muoversi.'),('EAU993XFSA','Deepstaria enigmatica',NULL,NULL,1967,'Queste meduse sono solite abitare profondità elevate. Possiedono una larga ma sottile campana trasparente, che può raggiungere anche i 2 metri di larghezza. Non possiedono tentacoli e vivono in solitudine.'),('EZH5BFUT5O','Lampocteis cruentiventer',NULL,'Ctenoforo dal ventre insanguinato',2001,'Questo ctenoforo presenta un colore rosso molto intenso e dei filamenti interni seghettati che vengono talvolta attraversati da particelle bio-luminose sconosciute. Al momento sono l\'unica specie conosciuta del loro genere.'),('JANN0QNIG4','Psychrolutes phrictus',NULL,'Blob sculpin',1978,'Questa specie di sculpin abita le profondità più oscure degli oceani. Presentano diverse spine, come tutte le altre specie di sculpin, ma presentano anche una corporatura più tozza.'),('KHRIIMYK0J','Marrus orthocanna',NULL,'Medusa sifonofora cintura di fuoco',1942,'Questa specie di sifonoforo presenta un colore rosso/arancio intenso, essendo i sifonofori dei raggruppamenti di organismi, può assumere diverse forme, caratterizzate solitamente da filamenti intrecciati.'),('LBPHU6GZV0','Enypniastes eximia',NULL,'Danzatore spagnolo',1882,'Questa specie di cetriolo marino nuota a profondità elevate. Presenta una colorazione accesa, solitamente rosso, rosa o arancione.'),('ORYC2H5PF4','Albatrossia pectoralis',NULL,'Coda di ratto gigante',1892,'Questo pesce è solito abitare il suolo marino. Possiede una corporatura tozza con una coda più sottile. Esemplari di questa specie possono crescere anche fino a 2 metri di lunghezza.'),('P4FPZBSA8O','Solmissus marshalli',NULL,'Medusa piatto da cena',1902,'Questa specie di medusa presenta numerosi e sottili tentacoli. La campana è quasi completamente trasparente. Sia i tentacoli che la campana sono fluorescenti e di colore bianco/azzurro.'),('UDPDW13YB8','Taonius borealis',NULL,'Calamaro cacatua',1972,'Questa specie di calamaro presenta un mantello e pinne trasparenti, che permettono la visibilità dall\'esterno degli organi interni. I tentacoli sono corti e bianchi, ma rossi sulla punta. Gli occhi sono di colore rosso mattone.'),('VKFBS2ECGG','Periphyllopsis braueri',NULL,NULL,1902,'Queste meduse presentano una campana spessa e di colore grigio/viola. I tentacoli sono molto sottili e genericamente di colore bianco. La campana presenta inoltre delle appendici che si muovono come i petali di un fiore, chiudendosi concentricamente per spingersi in avanti.'),('VL5L84606R',NULL,'Gambero pesante',NULL,2023,'Questa specie di gambero utilizza una dura corazza per proteggersi; questo comporta un maggiore peso, che non gli permette di nuotare come altre specie di gamberetti. Saranno necessari ulteriori studi.'),('WOCOAYY52M','Pycnogonid',NULL,'Ragno marino',1810,'Questi artropodi sono stati osservati a profondità elevate, gli esemplari possono essere di colore diverso e tutti posseggono otto zampe sottili, come i ragni comuni, e un torso piccolo.'),('WOXO3HBSWG','Hexanchus griseus',NULL,'Squalo vacca',1786,'Questo squalo presenta 6 paia di branchie e un muso schiacciato. Possono crescere fino a 6 metri di lunghezza.');
...
INSERT INTO `paesi` VALUES ('Alaska'),('Ecuador'),('Giappone'),('Indonesia'),('Messico'),('Papua Nuova Guinea');
...
INSERT INTO `prelievi` VALUES ('F448LXLYCN',1,'FKCHELYUK3',2300,'Le rocce sono state prelevate nelle vicinanze di un vulcano sottomarino.',1.70,'Rocce effusive');
...
CREATE TABLE `relitti` (
  ...
  `DataAffondamento` smallint DEFAULT NULL,
...
INSERT INTO `relitti` VALUES ('L8UNN',NULL,1950,38,'La barca risulta gravemente danneggiata, con diversi buchi su un fianco, possibile risultato di uno scontro in guerra');
...
CREATE TABLE `spedizioni` (
  ...
  CONSTRAINT `REF_SPEDI_GRUPP_FK` FOREIGN KEY (`NomeAssociazione`, `IDgruppo`) REFERENCES `gruppi_di_esplorazione` (`NomeAssociazione`, `Nome`),
...
INSERT INTO `spedizioni` VALUES ('FKBECBJLP3','2019-03-19','NOAA','QHK1T','Golfo del Messico','EQE2435'),('FKCHELYUK3','2023-07-02','Ocean Exploration Trust','WK8OQ','Fossa delle Aleutine','8GPK420'),('HAS08MN2FT','1997-07-13','JAMSTEC','6G11M','Mar di Banda','4GMW310'),('SSEY2O52BP','2023-07-21','Ocean Exploration Trust','WK8OQ','Fossa delle Aleutine','8GPK420'),('UI13BGB839','2022-05-15','JAMSTEC','4XPCU','Oceano Indiano','EQE2435'),('VJ4EIJACWK','2023-06-18','Ocean Exploration Trust','WK8OQ','Fossa delle Aleutine','VT16752');
...
-- Dump completed on 2023-08-22 16:54:10
